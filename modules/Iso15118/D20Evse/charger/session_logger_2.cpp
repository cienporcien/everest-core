...
using LogEvent = iso15118::session_2::logging::Event;
...
std::ostream& operator<<(std::ostream& os, const iso15118::session_2::logging::ExiMessageDirection& direction) {
    using Direction = iso15118::session_2::logging::ExiMessageDirection;
    switch (direction) {
...
public:
    ...
    void operator()(const iso15118::session_2::logging::SimpleEvent& event) {
        file << "- type: INFO\n";
        add_timestamp(event.time_point);
        file << "  info: \"" << event.info << "\"\n";
    }

    void operator()(const iso15118::session_2::logging::ExiMessageEvent& event) {
        file << "- type: EXI\n";
        add_timestamp(event.time_point);
...
private:
    ...
    iso15118::session_2::logging::TimePoint last_timestamp;
...
SessionLogger::SessionLogger(std::filesystem::path output_dir_) : output_dir(std::filesystem::absolute(output_dir_)) {
    ...
    iso15118::session_2::logging::set_session_log_callback([this](std::uintptr_t id, const LogEvent& event) {
